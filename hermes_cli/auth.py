...
def resolve_provider(
    ...
    *,
    ...
    _PROVIDER_ALIASES = {
        ...
        "kiro": "kiro-acp", "kiro-cli": "kiro-acp", "kiro-acp-agent": "kiro-acp",
...
def _resolve_external_process_command(provider_id: str) -> tuple:
    """Return (command, args) for an external-process provider."""
    if provider_id == "kiro-acp":
        command = os.getenv("HERMES_KIRO_ACP_COMMAND", "").strip() or "kiro-cli"
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            f"Could not find the command '{command}' for provider '{pconfig.name}'. "
            f"Install it or set the appropriate environment variable.",
            provider=provider_id,
            code="missing_cli",
        )
