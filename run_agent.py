...
class AIAgent:
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        if self.provider == "copilot-acp" or str(client_kwargs.get("base_url", "")).startswith("acp://copilot"):
            ...
            return client
        if self.provider == "kiro-acp" or str(client_kwargs.get("base_url", "")).startswith("acp://kiro"):
            from agent.kiro_acp_client import KiroACPClient

            client = KiroACPClient(**client_kwargs)
            logger.info(
                "Kiro ACP client created (%s, shared=%s) %s",
                reason,
                shared,
                self._client_log_context(),
            )
    ...
    def _create_request_openai_client(self, *, reason: str) -> Any:
        ...
        # ACP clients maintain persistent connections — reuse the primary client
        if self.provider in ("kiro-acp", "copilot-acp"):
            return primary_client
        with self._openai_client_lock():
    ...
    def _close_request_openai_client(self, client: Any, *, reason: str) -> None:
        # Don't close persistent ACP clients — they're the shared primary
        if self.provider in ("kiro-acp", "copilot-acp"):
            return
